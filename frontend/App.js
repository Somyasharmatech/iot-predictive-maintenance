...
const generateMockData = (numPoints = 50) => {
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    // Simulate an anomaly after a certain point
...
const getPrediction = async (data) => {
  ...
  return {
    ...
    probability_of_normal: 1 - probability,
  };
};
...
const App = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setData(prevData => {
        ...
        if (Math.random() > 0.98) { // 2% chance of a high vibration spike
  ...
  const getStatusColor = (value) => {
    if (value > 0.7) return "#ef4444"; // Red for high risk
    if (value > 0.4) return "#eab308"; // Yellow for medium risk
    return "#22c55e"; // Green for low risk
  };

  return (
    <div className="bg-slate-900 text-slate-100 p-8 min-h-screen font-sans">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-bold mb-8 text-center text-indigo-400">
          IoT Predictive Maintenance Dashboard
        </h1>

        {/* User Input and Prediction Form */}
        <div className="bg-slate-800 rounded-xl shadow-lg p-6 mb-8">
          <h2 className="text-2xl font-semibold mb-6 text-indigo-400">Manual Prediction</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            {prediction ? (
              <>
                ...
                <p className="text-lg text-center text-slate-400">
                  Model confidence: {(prediction.probability_of_failure * 100).toFixed(2)}%
                </p>
              </>
            ) : (
          ...
          <div className="bg-slate-800 rounded-xl shadow-lg p-6 flex flex-col justify-center items-center">
            <h2 className="text-xl font-semibold mb-4 text-indigo-400">Probability of Failure</h2>
            {prediction && (
          ...
          <div className="bg-slate-800 rounded-xl shadow-lg p-6 flex flex-col justify-between">
            <div className="flex items-center space-x-4 mb-4">
              <TachometerIcon />
              <h2 className="text-xl font-semibold">Latest Vibration</h2>
            </div>
            {data.length > 0 && (
